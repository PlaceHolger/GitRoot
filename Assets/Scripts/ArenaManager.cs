...
public class ArenaManager: MonoBehaviour
{
    ...
    [SerializeField]
    private List<GameObject> decorationScatterPrefabs;

    private List<GameObject> borderElements = new List<GameObject>();

    private List<GameObject> spawnPoints = new List<GameObject>();
    public List<GameObject> ArenaSpawnPoints => spawnPoints;
    ...
    public ArenaGridElement GetRandomGridElement()
    {
        return arenaGridElements.ElementAt(Random.Range(0, arenaGridElements.Count)).Value;
    }
        ...
        foreach ( var contentInformation in arenaGridContent.content )
        {
            ...
            if ( classification.go != null )
            {
                ...
                if ( classification.go != arenaEmptyGround && classification.go != arenaBorderPrefabs[2] )
                {
                    ...
                    arenaGridElement.additionalGameObject.isStatic = true;
                    ...
                    for (int k = 0; k < 1 + (randFac / 2); k++)
                    {
                        if ( Random.Range(0, 2 * (arenaGridBorder + 1) ) < randFac )
                        {
                            ...
                            if (decorationTreePrefabs.Count > 0)
                            {
                                ...
                                decorationObject.isStatic = true;
                            }
                        }
                    }
                }
